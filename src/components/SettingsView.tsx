  ...
  const SectionCard = ({ id, title, icon, children }: { id: string; title: string; icon?: string; children: React.ReactNode }) => (
    <div className="bg-white rounded-2xl border border-warm-200 overflow-hidden">
      <button
        ...
        className="w-full px-4 py-3.5 flex items-center justify-between hover:bg-warm-50 transition-all"
      >
        <span className="text-sm font-semibold text-warm-800">{icon && `${icon} `}{title}</span>
        <span className="text-warm-400 text-xs">{expandedSection === id ? '▲' : '▼'}</span>
      </button>
      {expandedSection === id && (
    ...
    <div className="px-5 py-6 space-y-4 pb-24">
      ...
      <div className="bg-white rounded-2xl border border-warm-200 p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-warm-800">{name || 'User'}</p>
            <p className="text-xs text-warm-400">{email}</p>
          </div>
        </div>
      </div>

      {/* Desktop: 2-column grid for settings sections */}
      ...
      <SectionCard id="body" title="Body Stats" icon="📊">
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 rounded-xl border border-warm-200 focus:border-brand-500 focus:ring-2 focus:ring-brand-100 outline-none text-sm"
          />
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <input type="number" value={age} onChange={e => setAge(Number(e.target.value))}
              className="w-full px-3 py-2 rounded-xl border border-warm-200 focus:border-brand-500 outline-none text-sm" />
          </div>
          <div>
            <label className="block text-xs font-medium text-warm-500 mb-1">Height (cm)</label>
            <input type="number" value={height} onChange={e => setHeight(Number(e.target.value))}
              className="w-full px-3 py-2 rounded-xl border border-warm-200 focus:border-brand-500 outline-none text-sm" />
          </div>
          <div>
            <label className="block text-xs font-medium text-warm-500 mb-1">Weight (kg)</label>
            ...
            {(['female', 'male', 'other'] as const).map(g => (
              <button key={g} onClick={() => setGender(g)}
                className={`flex-1 py-2 rounded-xl text-sm font-medium transition-all ${
                  gender === g ? 'bg-brand-500 text-white' : 'bg-warm-100 text-warm-600 hover:bg-warm-200'
                }`}>
      ...
      <SectionCard id="activity" title="Activity & Goal" icon="🏃">
        <div>
          <label className="block text-xs font-medium text-warm-500 mb-2">Activity level</label>
        ...
        <div>
          <label className="block text-xs font-medium text-warm-500 mb-2">Goal</label>
            ...
            {[
              ...
              <button key={opt.value} onClick={() => setGoal(opt.value)}
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${
                  goal === opt.value ? 'bg-brand-500 text-white' : 'bg-warm-100 text-warm-600 hover:bg-warm-200'
                }`}>
                {opt.label}
              </button>
            ))}
      ...
      <SectionCard id="diet" title="Food Preferences" icon="🥗">
        <div>
          <label className="block text-xs font-medium text-warm-500 mb-2">Diet type</label>
            ...
            {[
              ...
              <button key={opt.value} onClick={() => setDietaryPreference(opt.value)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                  dietaryPreference === opt.value ? 'bg-brand-500 text-white' : 'bg-warm-100 text-warm-600 hover:bg-warm-200'
                }`}>
                {opt.label}
              </button>
            ))}
        ...
        <div>
          <label className="block text-xs font-medium text-warm-500 mb-2">Allergies</label>
          <div className="flex flex-wrap gap-1.5">
            {COMMON_ALLERGIES.map(a => (
              <button key={a} onClick={() => toggleItem(allergies, setAllergies, a)}
                className={`px-2.5 py-1 rounded-full text-xs font-medium transition-all ${
                  allergies.includes(a) ? 'bg-red-100 text-red-700 border border-red-300' : 'bg-warm-100 text-warm-600'
                }`}>
                {a}
              </button>
            ))}
          ...
          <div className="flex gap-2 mt-2">
            <input type="text" value={customAllergy} onChange={e => setCustomAllergy(e.target.value)}
              placeholder="Other allergy..." className="flex-1 px-3 py-1.5 rounded-lg border border-warm-200 text-sm outline-none focus:border-brand-500"
            ...
            <button onClick={() => { if (customAllergy.trim()) { setAllergies([...allergies, customAllergy.trim()]); setCustomAllergy(''); } }}
              className="px-3 py-1.5 bg-warm-100 rounded-lg text-sm hover:bg-warm-200">Add</button>
          </div>
        </div>
        <div>
          <label className="block text-xs font-medium text-warm-500 mb-2">Dislikes</label>
          <div className="flex flex-wrap gap-1.5">
            {COMMON_DISLIKES.map(d => (
              <button key={d} onClick={() => toggleItem(dislikes, setDislikes, d)}
                className={`px-2.5 py-1 rounded-full text-xs font-medium transition-all ${
                  dislikes.includes(d) ? 'bg-teal-100 text-teal-600 border border-teal-500' : 'bg-warm-100 text-warm-600'
                }`}>
                {d}
              </button>
            ))}
          ...
          <div className="flex gap-2 mt-2">
            <input type="text" value={customDislike} onChange={e => setCustomDislike(e.target.value)}
              placeholder="Other dislike..." className="flex-1 px-3 py-1.5 rounded-lg border border-warm-200 text-sm outline-none focus:border-brand-500"
      ...
      <SectionCard id="kitchen" title="Kitchen Setup" icon="🍳">
        <div>
          <label className="block text-xs font-medium text-warm-500 mb-2">Cuisines</label>
          <div className="grid grid-cols-2 gap-2">
            {CUISINE_OPTIONS.map(opt => (
              <button key={opt.value}
                onClick={() => setFoodPalette(prev => prev.includes(opt.value) ? prev.filter(v => v !== opt.value) : [...prev, opt.value])}
                className={`px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                  foodPalette.includes(opt.value) ? 'bg-brand-500 text-white' : 'bg-warm-100 text-warm-600 hover:bg-warm-200'
                }`}>
        ...
        <div>
          <label className="block text-xs font-medium text-warm-500 mb-2">Cooking equipment</label>
          <div className="flex flex-wrap gap-1.5">
            {COOKING_EQUIPMENT.map(eq => (
              <button key={eq.id} onClick={() => toggleItem(cookingEquipment, setCookingEquipment, eq.label)}
                className={`px-2.5 py-1 rounded-full text-xs font-medium transition-all ${
                  cookingEquipment.includes(eq.label) ? 'bg-brand-100 text-brand-700 border border-brand-300' : 'bg-warm-100 text-warm-600'
                }`}>
                {eq.label}
              </button>
            ))}
        ...
        <div>
          <label className="block text-xs font-medium text-warm-500 mb-2">Meals per day</label>
          <div className="flex gap-2">
            {[1, 2, 3, 4].map(n => (
              <button key={n} onClick={() => setMealsPerDay(n)}
                className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all ${
                  mealsPerDay === n ? 'bg-brand-500 text-white' : 'bg-warm-100 text-warm-600 hover:bg-warm-200'
                }`}>
                {n}
              </button>
            ))}
      ...
      <SectionCard id="pantry" title="Pantry Staples" icon="🏪">
        <div className="rounded-xl p-2 bg-warm-50">
          <label className="block text-xs font-medium text-warm-600 mb-1.5">Always in my kitchen</label>
        ...
        <div className="rounded-xl p-2 bg-warm-50">
          <label className="block text-xs font-medium text-warm-600 mb-1.5">Usually have</label>
          <div className="flex flex-wrap gap-1">
            {pantryUsually.map(item => (
              <span key={item} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-teal-100 text-teal-600">
                {item}
                <button onClick={() => setPantryUsually(pantryUsually.filter(i => i !== item))} className="text-teal-500 hover:text-red-600">&times;</button>
              </span>
            ))}
          </div>
        </div>
        <div className="flex gap-2 items-center">
          <input type="text" value={customPantryItem} onChange={e => setCustomPantryItem(e.target.value)}
            placeholder="Add item..." className="flex-1 px-3 py-1.5 rounded-lg border border-warm-200 text-sm outline-none focus:border-brand-500"
            onKeyDown={e => {
          ...
          <select value={addToTier} onChange={e => setAddToTier(e.target.value as 'always' | 'usually')}
            className="px-2 py-1.5 rounded-lg border border-warm-200 text-xs outline-none bg-white">
      ...
      <div className="bg-white rounded-2xl border border-warm-200 p-4 space-y-3">
        <p className="text-sm font-semibold text-warm-800">Nutrition Targets</p>
        ...
        <p className="text-[10px] text-warm-400">Calculated using the Mifflin-St Jeor equation</p>
      </div>
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-5 backdrop-overlay animate-fade-in">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-xs p-6 space-y-4">
            <h3 className="text-lg font-bold text-warm-900">Log out?</h3>
            <p className="text-sm text-warm-500">Your settings and preferences are saved. You can log back in anytime.</p>
      ...
      {/* Sticky save bar */}
      {hasChanges && (
        <div className="fixed bottom-0 left-0 right-0 z-50 p-4 mx-auto max-w-4xl bg-surface-cream/90 backdrop-blur-sm border-t border-warm-100">
          <button
            onClick={handleSave}
            disabled={isSaving}
